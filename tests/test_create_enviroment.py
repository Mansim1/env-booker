...
def client(tmp_path):
    ...
    with app.test_client() as c:
        # login as admin
        c.post("/auth/login", data={"email":"admin@example.com","password":"AdminPass123!"})
        yield c
...
@pytest.mark.parametrize("name,owner,message", [
...
def test_unique_name_enforced(client):
    # create first
    client.post("/environments/new", data={"name":"Unique","owner_squad":"Team1"}, follow_redirects=True)
    # attempt duplicate
    resp = client.post(
        "/environments/new",
